...
object Huffman {
   ...
   */
  def until(isSingleton: List[CodeTree] => Boolean, combinator: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): CodeTree =
    if (isSingleton(trees)) trees.head
    else until(isSingleton, combinator)(combinator(trees))
  ...
  def createCodeTree(chars: List[Char]): CodeTree = {
    val t = times(chars)
    val m = makeOrderedLeafList(t)
    until(singleton, combine)(m)
  }
